...
#else
    IDirect3DVertexBuffer9    *pOrigVertexBuffer;
#endif
...
#else
        if(FAILED(pOrigVertexBuffer->Lock(0, 0, (void**)&pCalculateData, 0)))
#endif
        {
										   ...
										   bool			   *pbFatalError)
{
    ...
    {
#ifndef D3D9
        g_pD3DDevice->GetStreamSource(uiStream, &pOrigVertexBuffer, &uiStride);
#else
        g_pD3DDevice->GetStreamSource(uiStream, &pOrigVertexBuffer, 0, &uiStride);
#endif
...
#ifndef D3D9
                g_pD3DDevice->SetStreamSource(uiStream, pCachedStream->Stream.pPatchedStream, pCachedStream->Stream.uiNewStride);
#else
                g_pD3DDevice->SetStreamSource(uiStream, pCachedStream->Stream.pPatchedStream, 0, pCachedStream->Stream.uiNewStride);
#endif
...
#else
    IDirect3DVertexBuffer9    *pOrigVertexBuffer;
    IDirect3DVertexBuffer9    *pNewVertexBuffer;
#endif
    uint08                    *pOrigData;
    uint08                    *pNewData;
    UINT                       uiStride;
...
#else
        if(FAILED(pOrigVertexBuffer->Lock(0, 0, (void**)&pOrigData, 0)))
#endif
        {
            CxbxKrnlCleanup("Couldn't lock the original buffer");
        }
#ifndef D3D9
        g_pD3DDevice->CreateVertexBuffer(dwNewSize, 0, 0, XTL::D3DPOOL_MANAGED, &pNewVertexBuffer);
        if(FAILED(pNewVertexBuffer->Lock(0, 0, &pNewData, 0)))
#else
        g_pD3DDevice->CreateVertexBuffer(dwNewSize, 0, 0, XTL::D3DPOOL_MANAGED, &pNewVertexBuffer, NULL);
        if(FAILED(pNewVertexBuffer->Lock(0, 0, (void**)&pNewData, 0)))
#endif
        {
...
#else
        if(FAILED(g_pD3DDevice->SetStreamSource(uiStream, pNewVertexBuffer, 0, pStreamPatch->ConvertedStride)))
#endif
        {
...
#ifndef D3D9
        g_pD3DDevice->CreateVertexBuffer(Desc.Size, 0, 0, XTL::D3DPOOL_MANAGED, &pNewVertexBuffer);
        if(FAILED(pNewVertexBuffer->Lock(0, 0, &pData, 0)))
#else
        g_pD3DDevice->CreateVertexBuffer(Desc.Size, 0, 0, XTL::D3DPOOL_MANAGED, &pNewVertexBuffer, NULL);
        if(FAILED(pNewVertexBuffer->Lock(0, 0, (void**)&pData, 0)))
#endif
        {
...
#else
        if(FAILED(g_pD3DDevice->SetStreamSource(uiStream, pNewVertexBuffer, 0, uiStride)))
#endif
        {
                                        ...
                                        UINT             uiStream)
{
    ...
    {
#ifndef D3D9
        g_pD3DDevice->GetStreamSource(0, &pStream->pOriginalStream, &pStream->uiOrigStride);
#else
        g_pD3DDevice->GetStreamSource(0, &pStream->pOriginalStream, 0, &pStream->uiOrigStride);
#endif
...
#ifndef D3D9
        g_pD3DDevice->CreateVertexBuffer(dwNewSizeWR, 0, 0, XTL::D3DPOOL_MANAGED, &pStream->pPatchedStream);
#else
        g_pD3DDevice->CreateVertexBuffer(dwNewSizeWR, 0, 0, XTL::D3DPOOL_MANAGED, &pStream->pPatchedStream, NULL);
#endif
...
#else
            pStream->pOriginalStream->Lock(0, 0, (void**)&pOrigVertexData, 0);
#endif
        }
...
#else
            pStream->pPatchedStream->Lock(0, 0, (void**)&pPatchedVertexData, 0);
#endif
        }
    }
    else
...
#ifndef D3D9
        g_pD3DDevice->SetStreamSource(0, pStream->pPatchedStream, pStream->uiOrigStride);
#else
        g_pD3DDevice->SetStreamSource(0, pStream->pPatchedStream, 0, pStream->uiOrigStride);
#endif
    }
...
bool XTL::VertexPatcher::Restore()
{
    ...
    for(UINT uiStream = 0; uiStream < m_uiNbrStreams; uiStream++)
    {
        ...
        {
#ifndef D3D9
            g_pD3DDevice->SetStreamSource(0, m_pStreams[uiStream].pOriginalStream, m_pStreams[uiStream].uiOrigStride);
#else
            g_pD3DDevice->SetStreamSource(0, m_pStreams[uiStream].pOriginalStream, 0, m_pStreams[uiStream].uiOrigStride);
#endif
        }
