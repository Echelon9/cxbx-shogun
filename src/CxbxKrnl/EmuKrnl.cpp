...
// ******************************************************************
XBSYSAPI EXPORTNUM(129) uint08 NTAPI xboxkrnl::KeRaiseIrqlToDpcLevel( VOID )
{
    EmuSwapFS();   // Win2k/XP FS
	...
	// blueshogun 4/14/2007
	// I just stuck this here as a DOAV and Unreal Championship hack
	EmuWarning( "KeRaiseIrqlToDpcLevel: Ignored!\n" );

    EmuSwapFS();   // Xbox FS

	// blueshogun96 7/31/2007
	// I have no idea what actual IRQL should be returned.  Any suggestions
	// are welcome on this!
    return 0;
}
...
// ******************************************************************
XBSYSAPI EXPORTNUM(169) xboxkrnl::PVOID NTAPI xboxkrnl::MmCreateKernelStack( BOOLEAN GuiStack )
{
	EmuSwapFS();	// Win2k/XP FS
	...
	DbgPrintf("EmuKrnl (0x%X): MmCreateKernelStack\n"
			...
			"	GuiStack				: 0x%.08X\n"
			");\n",
			GetCurrentThreadId(), GuiStack );

	// blueshogun96 4/07/08 9:58PM
	// TODO: Implement a "proper" algorithm for this.  I just added
	// it for DOAV because it appears to be calling it directly somehow
	// rather than through an XAPI function call.  I kinda whipped it up
	// at the last minute, lol
	PVOID KernelStack = NULL;
	DWORD dwStackSize = GuiStack ? 0xF000 : 0x6000;

//	dwStackSize /= 0x1000;
	KernelStack = CxbxMalloc(dwStackSize);

//	EmuWarning( "MmCreateKernelStack() is temporarily being ignored!" );
//	__asm int 3;
...
)
{
	...
	DbgPrintf("EmuKrnl (0x%X): MmDeleteKernelStack\n"
			...
			"	GuiStack				: 0x%.08X\n"
			");\n",
			GetCurrentThreadId(), &Stack, Stack, GuiStack ); 

	CxbxFree( Stack );

//	EmuWarning( "MmDeleteKernelStack() is temporarily being ignored!" );
...
)
{
    ...
    if(szBuffer != NULL)
    {
		...
		// Adding support for when the program specifies the harddrive partition.
		// This will fix one of our many problems with DOAV!
		if( szBuffer[0] == '\\' && szBuffer[1] == 'D' )
		{
			char szTempBuffer[36];

			::memset(szTempBuffer, 0, 36);
			::memcpy(szTempBuffer, szBuffer, 35);

			if(!strstr(szTempBuffer, "\\Device\\Harddisk0\\Partition1\\TDATA\\"))
			{
				szBuffer += 35;
			...
			else if(!strstr(szTempBuffer, "\\Device\\Harddisk0\\Partition1\\UDATA\\"))
			{
				szBuffer += 35;
...
// ******************************************************************
XBSYSAPI EXPORTNUM(193) NTSTATUS NTAPI xboxkrnl::NtCreateSemaphore
(
...
)
{
	...
	DbgPrintf("EmuKrnl (0x%X): NtCreateSemaphore\n"
		   ...
		   "   MaximumCount        : 0x%.08X\n"
           ");\n",
           GetCurrentThreadId(), SemaphoreHandle, DesiredAccess, InitialCount, MaximumCount);

	// blueshogun96 10/2/07
	// I comment and date my work to mark my progress so I don't forget what I did :)
	// This will solve one problem with Prince of Persia: Sands of time (XDK 5558)

	// Redirect to NtCreateSemaphore
	NTSTATUS ret = NtDll::NtCreateSemaphore( SemaphoreHandle, DesiredAccess, NULL, InitialCount, MaximumCount );
...
// ******************************************************************
XBSYSAPI EXPORTNUM(206) NTSTATUS NTAPI xboxkrnl::NtQueueApcThread
(
...
)
{
	...
	DbgPrintf("EmuKrnl (0x%X): NtQueueApcThread\n"
			...
			"	ApcReserved			: 0x%.08X\n"
			");\n",
			GetCurrentThreadId(), ThreadHandle, ApcRoutine, ApcRoutineContext, ApcStatusBlock, ApcReserved );

	// blueshogun96 4/26/08
	// Metal Slug 3
	NTSTATUS ret;

	ret = NtDll::NtQueueApcThread( (NtDll::HANDLE)ThreadHandle, 
...
// ******************************************************************
XBSYSAPI EXPORTNUM(217) NTSTATUS NTAPI xboxkrnl::NtQueryVirtualMemory
(
...
)
{
	...
	DbgPrintf("EmuKrnl (0x%X): NtQueryVirtualMemory\n"
           ...
           "   pBuffer               : 0x%.08X\n"
           ");\n",
           GetCurrentThreadId(), pBaseAddress, pBuffer);

	// blueshogun96 5/14/2007 (my birthday!!)
	// Added for Unreal Championship.  
//	EmuWarning( "NtQueryVirtualMemory is temporarily being ignored!\n" );
//	__asm int 3;

	ULONG ResultLength;
	NTSTATUS ret = NtDll::NtQueryVirtualMemory( GetCurrentProcess(),
												pBaseAddress,
												NtDll::MemoryBasicInformation,
												pBuffer,
												sizeof( *(DWORD*) pBuffer ), // TODO: Sorry, but that was the best
																	// I could come up with atm, in a rush!
												&ResultLength );
	if( ResultLength != sizeof( *(DWORD*) pBuffer ) )
		EmuWarning( "NtQueryVirtualMemory: sizeof( *pBuffer ) != ResultLength!\n" );
	...
	// blueshogun96 7/31/07
	// Update, forgot to return the error code!
	return ret;
}
...
// ******************************************************************
XBSYSAPI EXPORTNUM(222) NTSTATUS NTAPI xboxkrnl::NtReleaseSemaphore
(
...
{	
	...
	// blueshogun96 10/9/07
	// This should not only resolve some problems in Prince of Persia,
	// but also Dr. Muto (4928)
	NTSTATUS ret;
