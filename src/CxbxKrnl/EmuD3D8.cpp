        ...
        case X_D3DCOMMON_TYPE_TEXTURE:
        {
            ...
            if(X_Format == 0x2E)
            {
                EmuWarning("D3DFMT_LIN_D24S8 not yet supported!\n"
					"D3DFMT_LIN_D24S8 -> D3DFMT_A8R8G8B8\n");
                X_Format = 0x12;
            ...
            else if(X_Format == 0x00 /* X_D3DFMT_L8 */ || X_Format == 0x0B /* X_D3DFMT_P8 */ || X_Format == 0x01 /* X_D3DFMT_AL8 */ || 
				X_Format == 0x19 /* X_D3DFMT_A8*/ || // blueshogun96 10/1/07, lensflare demo
				X_Format == 0x1A /* X_D3DFMT_A8L8 */)
            {
                bSwizzled = TRUE;
...
// ******************************************************************
// * func: EmuIDirect3DVolumeTexture8_GetLevelDesc
// ******************************************************************
HRESULT WINAPI XTL::EmuIDirect3DVolumeTexture8_GetLevelDesc
(
	X_D3DVolumeTexture *pThis,
	UINT				Level,
	X_D3DSURFACE_DESC  *pDesc
)
{
	...
	DbgPrintf("EmuD3D8 (0x%X): EmuIDirect3DVolumeTexture8_GetLevelDesc\n"
           "(\n"
           "   pThis               : 0x%.08X\n"
           "   Level               : 0x%.08X\n"
           "   pDesc               : 0x%.08X\n"
           ");\n",
           GetCurrentThreadId(), pThis, Level, pDesc);

	//EmuVerifyResourceIsRegistered(pThis);

	//IDirect3DVolumeTexture8* pVolumeTexture8 = pThis->EmuVolumeTexture8;

	//HRESULT hRet = pVolumeTexture8->GetLevelDesc( Level, pDesc );
...
)
{
	...
	EmuWarning("SetScreenSpaceOffset ignored!\n");
...
// * func: EmuIDirect3DDevice8_GetDeviceCaps
// ******************************************************************
HRESULT WINAPI EmuIDirect3DDevice8_GetDeviceCaps
(
	XTL::D3DCAPS8* pCaps
)
{
	...
	// blueshogun96 8/8/08
	// D3DCAPS8 apprears to be the same for both PC and Xbox.
	// Later on it might be a good idea to fill in the values
	// manually with what the Xbox would return.  For now, this
	// just fixes anything that uses ID3DXSprite interfaces.

	HRESULT hr = g_pD3DDevice8->GetDeviceCaps( pCaps );
